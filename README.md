...
Double-click **SmartPDFSorter-Setup-1.0.4.msi** and follow the prompts. Smart PDF
...
```powershell
msiexec /i SmartPDFSorter-Setup-1.0.4.msi /qn /norestart
```
...
```powershell
msiexec /i SmartPDFSorter-Setup-1.0.4.msi /qn /norestart INSTALLDESKTOPSHORTCUT=1
```

Install somewhere else:

```powershell
msiexec /i SmartPDFSorter-Setup-1.0.4.msi /qn /norestart INSTALLFOLDER="D:\Apps\AS Resume Sorter"
```

Uninstall silently:

```powershell
msiexec /x SmartPDFSorter-Setup-1.0.4.msi /qn /norestart
```
...
If you can't install software, use **SmartPDFSorter-Portable-1.0.4.exe**. It runs
...
```
artifacts\
    SmartPDFSorter-Setup-1.0.4.msi        <- primary: install this on client PCs
    SmartPDFSorter-Portable-1.0.4.exe     <- secondary: no installation
    SHA256SUMS.txt
```
